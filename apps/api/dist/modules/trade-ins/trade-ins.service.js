"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TradeInsService = void 0;
const common_1 = require("@nestjs/common");
const trade_ins_repository_1 = require("./trade-ins.repository");
let TradeInsService = class TradeInsService {
    tradeInsRepository;
    constructor(tradeInsRepository) {
        this.tradeInsRepository = tradeInsRepository;
    }
    async create(userId, dto) {
        return this.tradeInsRepository.create({
            status: 'SUBMITTED',
            user: { connect: { id: userId } },
            product: { connect: { id: dto.productId } },
        });
    }
    async findOne(id) {
        const tradeIn = await this.tradeInsRepository.findById(id);
        if (!tradeIn) {
            throw new common_1.NotFoundException(`Trade-In request ${id} not found`);
        }
        return tradeIn;
    }
    async approve(id) {
        const tradeIn = await this.findOne(id);
        if (tradeIn.status !== 'SUBMITTED') {
            throw new common_1.BadRequestException('Only pending trade-ins can be approved');
        }
        return this.tradeInsRepository.update(id, {
            status: 'APPROVED',
            approvedAt: new Date(),
        });
    }
};
exports.TradeInsService = TradeInsService;
exports.TradeInsService = TradeInsService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [trade_ins_repository_1.TradeInsRepository])
], TradeInsService);
