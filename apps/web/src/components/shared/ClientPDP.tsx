...
import { ReturnRiskWidget } from './ReturnRiskWidget';

export function ClientPDP({ listing, relatedListings, isSecondLife = false }: { listing: any, relatedListings: any[], isSecondLife?: boolean }) {
  ...
  const avgRating = totalReviews > 0 ? (reviews.reduce((acc: any, curr: any) => acc + curr.rating, 0) / totalReviews).toFixed(1) : "0.0";
    ...
    <div className="max-w-[1500px] mx-auto p-4 flex flex-col lg:flex-row gap-6 mt-4 pb-20">
      ...
      <div className="w-full lg:w-[40%] flex gap-4">
        ...
        <div className="flex flex-col gap-2 w-[50px]">
          {listing.product.images.map((img: any, idx: number) => (
            ...
            >
              <ImageWithFallback src={img.imageUrl} alt="Thumbnail" className="w-full h-auto object-contain aspect-square" />
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col">
          <div className="relative w-full aspect-square flex items-center justify-center bg-white border border-gray-100 rounded-md p-4">
            <ImageWithFallback src={mainImage} alt={listing.title} className="max-w-full max-h-full object-contain" />
      ...
      <div className="w-full lg:w-[40%] flex flex-col">
        {isSecondLife && (
          <div className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">
            Amazon Certified {listing.listingType.replace('_', ' ')}
          </div>
        )}
        ...
        <div className="flex items-center gap-4 mt-2 pb-2 border-b border-gray-200">
          <div className="flex items-center gap-1 text-[#007185]">
            {avgRating} <span className="text-[#ffa41c]">★★★★☆</span> <span className="text-gray-400">|</span> <a href="#reviews" className="hover:underline cursor-pointer">{totalReviews} ratings</a>
          </div>
        </div>
        ...
        <div className="flex items-start justify-between border-t border-b border-gray-200 py-3 mt-4 text-[11px] text-[#007185] text-center">
           ...
           <div className="flex flex-col items-center gap-1 w-[20%] hover:underline cursor-pointer">
             ...
             <span>{isSecondLife ? '6 Month Warranty' : '1 Year Warranty'}</span>
           </div>
           {isSecondLife && (
             <div className="flex flex-col items-center gap-1 w-[20%] hover:underline cursor-pointer">
        ...
        <div className="mt-4 border-b border-gray-200 pb-4">
          ...
          <div className="text-xs text-[#0f1111] space-y-1">
            ...
            <p><strong>Condition:</strong> {isSecondLife ? `Amazon Certified ${listing.listingType === 'PRE_OWNED' ? 'Pre-Owned' : 'Refurbished'}` : 'New'}</p>
          </div>
        </div>
        ...
        <div className="mt-4 border-b border-gray-200 pb-4">
          ...
          <ul className="list-disc pl-5 text-sm text-[#0f1111]">
             ...
             {isSecondLife && <li>Testing & Certification Document</li>}
             {!isSecondLife && <li>User Manual</li>}
          </ul>
        </div>

        {/* Reviews Section */}
        <div id="reviews" className="mt-8 border-t border-gray-200 pt-6 pb-6">
              ...
              {reviews.map((rev: any) => (
                <div key={rev.id} className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">{rev.user?.firstName || 'Amazon'} {rev.user?.lastName || 'Customer'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[#ffa41c] text-sm">{'★'.repeat(rev.rating)}{'☆'.repeat(5 - rev.rating)}</span>
                    <span className="text-sm font-bold">{rev.title}</span>
                  </div>
                  <div className="text-xs text-gray-500">Reviewed in India on {new Date(rev.createdAt).toLocaleDateString()}</div>
                  <div className="text-xs text-[#c7511f] font-bold">Verified Purchase</div>
      ...
      <div className="w-full lg:w-[20%]">
        <div className="border border-gray-300 rounded-lg p-4 bg-white shadow-sm sticky top-28">
          ...
          {/* AI Predictive Return Risk Widget */}
          <ReturnRiskWidget 
          ...
          <div className="text-xs mt-3 flex items-center justify-between text-[#0f1111]">
             ...
             <span className="font-medium text-right">{isSecondLife ? 'Amazon 2nd Chance' : 'Amazon'}</span>
          </div>
          <div className="text-xs mt-1 flex items-center justify-between text-[#0f1111]">
             <span className="text-gray-500">Sold by</span>
             <span className="font-medium text-right text-[#007185] hover:underline cursor-pointer">{isSecondLife ? (listing.seller?.firstName || 'Amazon Retail') : 'Amazon Retail'}</span>
          </div>
