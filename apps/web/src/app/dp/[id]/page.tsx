...
export default async function OGProductDetailPage({ 
  params,
  searchParams
}: { 
  ...
  const isSecondLifeListing = ref === 'base' ? false : (ref === 'second-life' ? true : listing.listingType !== 'NEW');

  // Find related products using Postgres Trigram similarity (AI-like semantic text matching)
  const listingTypeCondition = isSecondLifeListing ? "listing_type != 'NEW'" : "listing_type = 'NEW'";
  const similarIds: any[] = await prisma.$queryRawUnsafe(`
    SELECT id
    FROM "listings"
    WHERE id != $1::uuid AND status = 'ACTIVE' AND ${listingTypeCondition}
  ...
  return (
    <div className="min-h-screen bg-white">
      <AmazonNavbar forceSecondLife={isSecondLifeListing} />
      <ClientPDP listing={listing} relatedListings={relatedListings} isSecondLife={isSecondLifeListing} />
    </div>
  );
}
